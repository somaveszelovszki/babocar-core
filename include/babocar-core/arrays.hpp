 ...
 **/
template <typename T>
void copy(const T * const src, T * const dest, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        dest[i] = src[i];
    }
}
 ...
 **/
template <typename T>
int32_t indexOf(const T& item, const T * const ar, size_t arraySize, size_t startIdx = 0){
    int32_t idx = -1;
 ...
 **/
template <typename T>
bool contains(const T& item, const T * const ar, size_t arraySize) {
 ...
 **/
template <typename T>
void reverse(T * const ar, size_t size) {
