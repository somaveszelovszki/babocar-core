...
template <typename T> struct Point2 {
    ...
    bcr::Sign getAngleSign(const Vec2<T>& other, bcr::RotationDir dir) const;
...
template<typename T>
bcr::Sign Point2<T>::getAngleSign(const Vec2<T>& other, bcr::RotationDir dir) const {
    float32_t m;
    if(bcr::isZero(this->X)) {
        m = static_cast<float32_t>(this->Y > 0 ? -other.X : other.X);
    } else if(bcr::isZero(other.X)) {
    ...
    return bcr::sgn(m * static_cast<int8_t>(dir));
}
...
bool Point2<T>::isInside(const Point2<T>& a, const Point2<T>& b, const Point2<T>& c) const {
    ...
    const bcr::RotationDir dir = bcr::RotationDir::LEFT;
    const bcr::Sign sA = ab.getAngleSign(ap, dir), sB = bc.getAngleSign(bp, dir), sC = ca.getAngleSign(cp, dir);
