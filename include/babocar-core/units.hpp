...
public:
    ...
    _res_dim_class operator/(const _dim_class2& other) const {
        float32_t ratio = this->get<stored_unit_inst_t>() / other.template get<typename _dim_class2::stored_unit_inst_t>();
        return _res_dim_class::template from<div_unit_instance<stored_unit_inst_t, typename _dim_class2::stored_unit_inst_t>>(ratio);
    }
