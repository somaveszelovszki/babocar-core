...
TEST(units, unit_convert) {
    ...
    constexpr distance_t eps = meter_t(detail::COMMON_EQ_ABS_EPS);
    EXPECT_EQ(mm, mm);
    EXPECT_EQ(cm, cm);
    EXPECT_EQ(m, m);
    EXPECT_EQ(km, km);
...
TEST(units, unit_div) {
    ...
    EXPECT_NEAR(4.0f, cm_per_sec_t(v).get(), detail::COMMON_EQ_ABS_EPS);
...
TEST(units, unit_mul) {
    ...
    EXPECT_NEAR(2.0f, centimeter_t(s).get(), detail::COMMON_EQ_ABS_EPS);
